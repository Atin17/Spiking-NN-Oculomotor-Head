...
class NewBrain:
    def __init__(self, frameQueue_Left, frameQueue_Right, threshold):
    ...
    def run(self):
        ...
        while self.running:
            leftQueueLength = self.frameQueue_Left.size()
            rightQueueLength = self.frameQueue_Left.size()
            ...
            frame_Left = self.frameQueue_Left.remove_last()
            frame_Right = self.frameQueue_Right.remove_last()
            ...
            cv2.threshold(frame_Left, self.threshold, 255, cv2.THRESH_BINARY, dst=frame_Left)
            cv2.threshold(frame_Right, self.threshold, 255, cv2.THRESH_BINARY, dst=frame_Right)
            ...
            for point in locations_Left:
                #print("Point:{} Point[0]:{}, Point[0][0]:{}".format(point, point[0], point[0][0]))
                loc = f"{point[0][0]},{point[0][1]}"
                #print("loc:", loc)
                left_x_center += point[0][0]
                left_y_center += point[0][1]
                #print("InverseJson: ", self.inverseJson)
    ...
    def setControl(self, arbotix_control):
        self.control = arbotix_control
    ...
    def setReward(self, left_x_center, right_x_center, vertical_center):
        if Constants.instance().learning:
            ...
            # Down Reward
            new_reward = (-1.0 + (vertical_center >= 360) * 2.0) * (1 - (vertical_center - 360) / 360.0)
